# --- Configure environment using anaconda ---
# C:\Users\horyc>conda create -n dear python=3.10
# C:\Users\horyc>activate dear
# C:\Users\horyc>pip install dearpygui control
# C:\Users\horyc>python step_response_visual.py

# Use code folding to navigate through this file
# Use code folding to navigate through this file
# Use code folding to navigate through this file
...
omega_n = 1.0  # Replace with your desired value
zeta = 0.3    # Replace with your desired value
T = control.TransferFunction([omega_n ** 2], [1, 2 * zeta * omega_n, omega_n ** 2])

def plot_the_step_response(T):
    data_x , data_y = control.step_response(T)
    print('\t', control.step_info(T))

    dpg.set_value(f'stepresponse', [list(data_x), list(data_y)])
    dpg.fit_axis_data(f'tag_x_axis_step')
    dpg.fit_axis_data(f'tag_y_axis_step')

def update_omega(sender, app_data):
    global zeta, omega_n
    # zeta = dpg.get_value("tag_slide_zeta")
    omega_n= app_data
    T = control.TransferFunction([omega_n ** 2], [1, 2 * zeta * omega_n, omega_n ** 2])
    plot_the_step_response(T)
    print(f'{omega_n=}; {zeta=}')

def update_zeta(sender, app_data):
    global  omega_n, zeta
    zeta = app_data
    # omega_n = dpg.get_value("tag_slide_omega_n")
    T = control.TransferFunction([omega_n ** 2], [1, 2 * zeta * omega_n, omega_n ** 2])
    plot_the_step_response(T)
    print(f'{omega_n=}; {zeta=:.2f}')
...
def _on_demo_close(sender, app_data, user_daa):
...
def show_dem_demo(CONSOLE):
    ...
    with dpg.window(label="DEM Primary Window", height=200, width=200, tag="__demo_primary_window"):
        ...
        # dpg.configure_item("__demo_primary_window", **{"menubar": True})
        ...
        with dpg.tree_node(label="Step Response", show=True, default_open=True):

            with dpg.plot(label='Step Response', height=600, width=-1):
                # optionally create legend
                dpg.add_plot_legend()

                # REQUIRED: create x and y axes, set to auto scale.
                x_axis = dpg.add_plot_axis(dpg.mvXAxis, label='label_x', tag='tag_x_axis_step')
                y_axis = dpg.add_plot_axis(dpg.mvYAxis, label='label_y', tag='tag_y_axis_step')

                data_x , data_y = control.step_response(T)
                # series belong to a y axis. Note the tag name is used in the update function realtime_update_data
                dpg.add_line_series(x=list(data_x),y=list(data_y), label='Step Response', parent='tag_y_axis_step',  tag='stepresponse')
                # dpg.add_line_series(x=list(time),y=list(response), 
                #     label='DuckyWantedResult', parent='tag_y_axis_step', 
                #     tag='tag_realtime_plot1')

        s1 = dpg.add_slider_int(label="slider omega_n", default_value=10, min_value=1, max_value=100, callback=update_omega, tag='tag_slide_omega_n')
        s2 = dpg.add_slider_float(label="slider zeta", default_value=0.5, max_value=1.2, format="damping ratio = %.2f", callback=update_zeta, tag='tag_slider_zeta')
    ...
    with dpg.window(label="Dear PyGui Demo", width=800, height=800, on_close=_on_demo_close, pos=(100, 100), tag="__demo_id"):
        ...
        with dpg.collapsing_header(label="Widgets"):
            ...
            with dpg.tree_node(label="Simple Plots"):
                ...
                data1 = []
                for i in range(70):
                    data1.append(math.cos(3.14*6*i/180))
        ...
        with dpg.collapsing_header(label="Plots"):
            ...
            for i in range(100):
                ...
                cosdatay.append(0.5 + 0.75*math.cos(50*i/100))
        ...
        with dpg.collapsing_header(label="Advanced"):
            ...
            with dpg.tree_node(label="Manual Mutex Control"):
                ...
                with dpg.group(horizontal=True):
                    ...
                    s1 = dpg.add_slider_int(label="Why?")
                    s2 = dpg.add_slider_int(label="Why??")
                ...
                dpg.configure_item(s1, user_data=dpg.last_item())
                dpg.configure_item(s2, user_data=dpg.last_item())
